   ...
   return (
      <div className="p-4 lg:p-8 space-y-12 lg:space-y-16 max-w-7xl mx-auto pb-32">
         <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div>
               <h1 className="text-3xl lg:text-5xl font-black tracking-tighter text-foreground uppercase italic">Academic <span className="text-primary">Planner</span></h1>
               <p className="text-muted-foreground font-medium text-sm lg:text-lg">Even Semester 2025-26 Matrix</p>
            </div>

            <div className="flex bg-foreground/5 p-1 lg:p-1.5 rounded-full border border-border/10 items-center gap-2 lg:gap-4 w-full max-w-xs mx-auto lg:mx-0 lg:w-fit justify-between">
               <button
                  onClick={() => setActiveMonthIdx(prev => Math.max(0, prev - 1))}
                  disabled={activeMonthIdx === 0}
                  className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl flex items-center justify-center text-foreground hover:bg-foreground/5 disabled:opacity-20 transition-all flex-shrink-0"
               >
                  <ChevronLeft size={20} />
               </button>
               <span className="text-xs lg:text-sm font-black uppercase tracking-widest text-foreground flex-1 text-center whitespace-nowrap">
                  {currentMonth.month}
               </span>
               <button
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
            <div className="glass-card rounded-[2.5rem] p-6 lg:p-8 border-primary/10 flex items-center gap-6 bg-primary/5">
               <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center text-primary">
                  <CalendarIcon size={24} />
               </div>
               <div>
                  <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Current Date</p>
                  <p className="text-xl font-bold text-foreground uppercase italic">{dateStr} ({dayName})</p>
               </div>
            </div>
            
            <div className="glass-card rounded-[2.5rem] p-6 lg:p-8 border-primary/10 flex items-center gap-6 relative overflow-hidden group">
               <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${dayOrder > 0 ? 'bg-blue-500/10 text-blue-500 animate-pulse' : 'bg-foreground/5 text-muted-foreground'}`}>
                  <Zap size={24} />
               </div>
               <div>
                  <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Today's Status</p>
                  <p className={`text-xl font-bold uppercase italic ${dayOrder > 0 ? 'text-blue-500' : 'text-foreground'}`}>
                     {dayOrder > 0 ? `Day Order: ${dayOrder}` : (new Date().getDay() === 0 || new Date().getDay() === 6) ? "Weekend" : "Holiday"}
                  </p>
               </div>
            </div>

            <div className="glass-card rounded-[2.5rem] p-6 lg:p-8 border-primary/10 flex items-center gap-6">
               <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center text-primary">
                  <Star size={24} />
               </div>
               <div>
                  <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Next Holiday</p>
                  <p className="text-xl font-bold text-foreground uppercase italic">
                     {(() => {
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {currentMonth.days.map((day, i) => {
               ...
               return (
                  <div key={i} className={`glass-card rounded-[2.5rem] p-6 border-primary/10 relative overflow-hidden group transition-all hover:scale-[1.02] 
                     ...
                     <div className="flex justify-between items-start mb-4">
                        <div className="flex gap-3 items-start">
                           <div>
                              <p className={`text-3xl font-black italic leading-none ${activeToday ? 'text-primary' : isNonWorking ? 'text-red-500' : 'text-foreground'}`}>{day.dt}</p>
                     ...
                     <div className="mt-4">
                        {day.event ? (
                           <p className={`font-black leading-tight uppercase ${isNonWorking ? 'text-xl text-red-500 italic' : isCritical ? 'text-lg text-primary' : 'text-sm text-foreground/80'}`}>
                              {day.event}
                           </p>
                        ) : day.do !== "-" ? (
                     ...
                     <div className={`absolute -bottom-2 -right-2 w-12 h-12 ${activeToday ? 'text-primary/10' : isNonWorking ? 'text-red-500/5' : 'text-foreground/5'} group-hover:scale-125 transition-transform`}>
