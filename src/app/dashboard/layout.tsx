...
function InnerDashboardContent({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground selection:bg-primary/30 relative">
      ...
      <div className="fixed top-0 left-0 right-0 z-[50] lg:left-72 bg-background/80 backdrop-blur-xl border-b border-primary/10 h-16 flex items-center px-4 lg:px-8 justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className={`w-2 h-2 rounded-full animate-pulse ${loading ? 'bg-amber-500' : 'bg-primary'}`} />
            <span className="text-[10px] lg:text-[11px] font-bold text-foreground/40 tracking-widest uppercase italic">
        ...
        <div className="flex items-center gap-2 lg:gap-4">
          <button
            ...
            disabled={loading}
            className="w-8 h-8 flex items-center justify-center transition-all active:scale-95 text-foreground/40 hover:text-primary disabled:opacity-50"
            title="Force Sync"
          >
      ...
      <aside className={`
        fixed h-full bg-background z-[110] border-r border-primary/10 transition-transform duration-500 ease-out flex flex-col
      ...
      `}>
        ...
        <div className="lg:hidden absolute top-4 right-4">
          <button
            onClick={() => setIsOpen(false)}
            className="w-8 h-8 bg-foreground/5 rounded-lg flex items-center justify-center border border-border/10 transition-all active:scale-95"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="p-8 pt-6 lg:pt-8">
          <div className="flex items-center gap-3 group cursor-default">
            <div className="w-8 h-8 bg-primary rounded-full flex items-center justify-center shadow-2xl shadow-primary/20 group-hover:scale-110 transition-transform">
              <Pokeball className="w-5 h-5 text-foreground" />
            </div>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4 overflow-y-auto">
          {navItems.map((item) => (
            ...
            >
              <span className={`${pathname === item.href ? "text-foreground" : "text-primary/50 group-hover:text-primary"} transition-colors`}>
                <item.icon size={22} />
              </span>
              {item.label}
            </Link>
